...
4. ![](../../resources/process/searchpatterns/pattern-element-delete.png) — кнопка для удаления элемента из шаблона. Возможно удалить клавишей `Del`.
...
Откроем приложение **Калькулятор**, для этого напишим в командной строке конслои **mate-calc** и нажмем клавишу **Еnter**. Создадим в Студии новый проект и выполним следующие действия:

1. Добавим контейнер [**Присоединиться к приложению**](https://docs.primo-rpa.ru/primo-rpa/g\_elements/osnovnye-elementy/els\_desktop/el\_desktop\_attach) **!!!ОБНОВИТЬ ССЫЛКУ!!!**. В свойстве **Заголовок** напишите заголовок окна **"Калькулятор"**.
2. Поместим в контейнер элемент [**Клик мышью**](https://docs.primo-rpa.ru/primo-rpa/g\_elements/osnovnye-elementy/els\_uiinteraction/el\_click) **!!!ОБНОВИТЬ ССЫЛКУ!!!**.
...
> *В настоящее время для построения паттерна поиска можно полагаться только на значение следующих свойств **Name**, **Role**, **Description** и **Index** (Индекс)*, значения остальных свойств или не удается получить с использованием текущего механизма, или не являются уникальными при повторном запуске приложения. Например значение свойства **Path** является уникальным только для объекта окна, но не является уникальным для элементов принадлежащем окну*


![](../../resources/process/searchpatterns/селекторы-калькулятора2.png)
