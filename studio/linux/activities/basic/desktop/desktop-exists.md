...
> - Для демонстрации `Присутствия элемента` необходимо переключиться из режима калькулятора `Простой` в режим `Расширенный` во время запуска примера (конопка `cos` должна появиться).
