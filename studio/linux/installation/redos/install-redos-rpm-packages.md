...
- `Browser Extensions` - набор для установки на машины, где планируется запускать оркестровых роботов (см инструкцию Настройка машины робота на РЕД ОС `todo: добавить гиперссылку на документ`) с гиперссылкой например.
